...
@pytest.fixture
def sample_df():
    # Đọc cccd và so_dien_thoai là string để giữ leading zeros
    return pd.read_csv(
...
class TestAnonymization:
    ...
    def test_pii_not_in_output(self, anonymizer, sample_df):
        ...
        anonymized_cccds = df_anon["cccd"].astype(str).tolist()
        for original_cccd in sample_df["cccd"]:
            assert str(original_cccd) not in anonymized_cccds
